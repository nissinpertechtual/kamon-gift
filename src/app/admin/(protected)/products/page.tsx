        ...
        <div style={{ border: '0.5px solid #2a2f35' }}>
          ...
          >
            {['画像', '商品名', '価格', '素材', '公開', 'Sort', '操作'].map((h) => (
              <span key={h} style={{ fontSize: '9px', letterSpacing: '0.15em', color: '#828990', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                {h}
              </span>
            ))}
          </div>
          ...
          {(products as Product[]).map((product, i) => (
            <div
              ...
              style={{
                ...
                gridTemplateColumns: '60px 1fr 120px 80px 80px 80px 80px',
                padding: '12px 16px',
                borderBottom: i < products.length - 1 ? '0.5px solid #1b1f23' : 'none',
                alignItems: 'center',
              }}
            >
              {/* サムネイル */}
              <div style={{ width: '48px', height: '36px', background: '#202127', overflow: 'hidden', flexShrink: 0 }}>
                {product.images?.[0] ? (
                  ...
                  <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontSize: '8px', color: '#2c3137', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>—</span>
                  </div>
                )}
              </div>
              ...
              <div style={{ paddingRight: '12px' }}>
                <div style={{ fontSize: '13px', color: '#e9e7e1', fontWeight: 300, fontFamily: "'Zen Old Mincho', 'Hiragino Mincho ProN', 'Yu Mincho', 'Cormorant Garamond', Georgia, serif", marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {product.name_ja || product.name}
                </div>
                <div style={{ fontSize: '10px', color: '#5d636a', fontFamily: "'Cormorant Garamond', Georgia, serif", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {product.name}
                </div>
              </div>

              {/* 価格 */}
              <span style={{ fontSize: '12px', color: '#9aa0a6', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                {product.price != null ? `¥${product.price.toLocaleString()}` : 'お見積もり'}
              </span>

              {/* 素材 */}
              <span style={{ fontSize: '11px', color: '#828990', fontFamily: "'Cormorant Garamond', Georgia, serif", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {product.material}
              </span>
              ...
              {/* sort_order */}
              <span style={{ fontSize: '11px', color: '#5d636a', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                {product.sort_order}
              </span>
