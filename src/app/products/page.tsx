...
const filterLinkStyle = (isActive: boolean, gold = false) => ({
  ...
  border: `0.5px solid ${isActive ? (gold ? '#efece4' : '#9aa0a6') : gold ? '#2c3137' : '#2a2f35'}`,
  color: isActive ? (gold ? '#efece4' : '#5d636a') : gold ? '#8b9298' : '#5d636a',
  textDecoration: 'none',
  ...
  return (
    <div style={{ position: 'relative', background: '#0b0c0e', minHeight: '100vh' }}>
      ...
      >
        ...
        {/* エラー表示 */}
        {error && (
          <p style={{ textAlign: 'center', color: '#ff6b5e', fontSize: '12px', marginBottom: '32px' }}>
            商品の取得に失敗しました。
          </p>
        )}
